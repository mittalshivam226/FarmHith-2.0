...
export function PageShell({ children, sidebar, topbar, className = '' }: PageShellProps) {
  ...
  return (
    <div className="flex h-screen bg-slate-900 overflow-hidden">
      {/* Desktop sidebar */}
      {sidebar && (
        <aside className="hidden md:flex flex-col w-64 shrink-0 bg-slate-800 border-r border-slate-700 z-20">
          {sidebar}
        </aside>
      )}
      ...
      {sidebar && mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-64 bg-slate-800 shadow-xl z-50 animate-in fade-in slide-in-from-left-4 duration-200">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        {/* Topbar Wrapper */}
        {(topbar || sidebar) && (
          <header className="bg-slate-800/80 backdrop-blur-md border-b border-slate-700 z-10 shrink-0 h-14 md:h-16 flex items-center">
            <div className="flex items-center gap-3 px-4 w-full h-full">
              {sidebar && (
                <button
                  onClick={() => setMobileOpen(true)}
                  aria-label="Open navigation menu"
                  className="md:hidden p-1.5 rounded-md text-slate-400 hover:bg-slate-700 transition-colors"
                >
                  <Menu size={20} />
                </button>
              )}
