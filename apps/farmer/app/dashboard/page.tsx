    ...
    <div className="space-y-6">
      ...
      {(!loadingOrders && pendingOrders.length > 0) && (
        <Alert variant="warning" title="Action Required: Pending Orders">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-1">
            ...
            <Link href="/dashboard/marketplace" className="text-sm font-semibold text-warning-800 hover:underline">
              Review Orders &rarr;
            </Link>
          </div>
        </Alert>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={loadingSessions ? '—' : upcomingSessions.length}
          icon={<Users size={20} />}
          accent="teal"
        />
        <StatCard
          ...
          accent={pendingOrders.length > 0 ? 'amber' : 'teal'}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card padding="none" className="overflow-hidden flex flex-col h-full">
          <CardHeader className="flex flex-row items-center justify-between bg-slate-50 border-b border-slate-100 py-4 px-5">
            <CardTitle>Recent Soil Tests</CardTitle>
            <Link href="/dashboard/soil-test" className="text-xs font-semibold text-primary-600 hover:text-primary-700 transition-colors">
              View All
            </Link>
          </CardHeader>
          <div className="flex-1 flex flex-col">
            {loadingBookings ? (
              ...
              <EmptyState 
                ...
                action={<Link href="/dashboard/soil-test/book" className="text-sm font-semibold text-primary-600 hover:underline">Book Test</Link>}
              />
            ) : (
              <div className="divide-y divide-slate-100 flex-1">
                {soilBookings.slice(0, 4).map(booking => (
                  <Link
                    key={booking.id}
        ...
        <Card padding="none" className="overflow-hidden flex flex-col h-full">
          <CardHeader className="flex flex-row items-center justify-between bg-slate-50 border-b border-slate-100 py-4 px-5">
            ...
            <Link href="/dashboard/mitra" className="text-xs font-semibold text-primary-600 hover:text-primary-700 transition-colors">
              Find Mitra
            </Link>
          </CardHeader>
          <div className="flex-1 flex flex-col">
            {loadingSessions ? (
              ...
              <EmptyState 
                ...
                icon={<Users size={24} />}
                action={<Link href="/dashboard/mitra" className="text-sm font-semibold text-primary-600 hover:underline">Browse Experts</Link>}
              />
            ) : (
              <div className="divide-y divide-slate-100 flex-1">
                {mitraSessions.slice(0, 4).map(session => (
                  <div key={session.id} className="flex items-center justify-between px-5 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-900">{session.mitraName}</p>
      ...
      <Card padding="none" className="overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between bg-slate-50 border-b border-slate-100 py-4 px-5">
          ...
          <Link href="/dashboard/marketplace" className="text-xs font-semibold text-primary-600 hover:text-primary-700 transition-colors">
            Manage Listings
          </Link>
        </CardHeader>
        ...
        <div className="p-5 sm:p-6 grid lg:grid-cols-2 gap-8">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <FileText size={16} className="text-slate-400" /> My Active Listings
            </h3>
            {loadingListings ? (
              <CardSkeleton />
            ) : activeListings.length === 0 ? (
              <div className="py-8 px-4 text-center rounded-lg border border-dashed border-slate-200 bg-slate-50/50">
                <p className="text-sm text-slate-500 mb-2">You don't have any active crop residue listings.</p>
                <Link href="/dashboard/marketplace/list" className="text-sm font-semibold text-primary-600 hover:underline">
                  Create Listing &rarr;
                </Link>
              </div>
            ) : (
              <div className="space-y-3">
                {activeListings.slice(0, 3).map(listing => (
                  <div key={listing.id} className="p-4 rounded-lg border border-slate-200 bg-white shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">{listing.residueType}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{listing.quantityTons} tons · Available {formatDate(listing.availableFrom)}</p>
                    </div>
                    <div className="flex flex-col sm:items-end">
                      <p className="text-sm font-bold text-slate-900">{formatCurrency(listing.farmhithPricePerTon)}<span className="text-xs font-normal text-slate-500">/ton</span></p>
                      <div className="mt-1">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
              <TrendingUp size={16} className="text-slate-400" /> Incoming Orders
            </h3>
            {loadingOrders ? (
              <CardSkeleton />
            ) : orders.length === 0 ? (
              ...
              <div className="space-y-3">
                {orders.slice(0, 3).map(order => (
                  <div key={order.id} className="p-4 rounded-lg border border-slate-200 bg-white shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">{order.plantName}</p>
                      <p className="text-xs text-slate-500 mt-0.5">
