...
export default function FarmerDashboard() {
  ...
  // Stats
  const activeTests   = mockSoilTestBookings.filter(b => b.status !== 'COMPLETED' && b.status !== 'CANCELLED').length;
  const upcomingSessions = mockMitraBookings.filter(b => b.status === 'CONFIRMED').length;
  const activeListings = mockCropListings.filter(l => l.status === 'ACTIVE').length;
  const pendingOrders  = mockProcurementOrders.filter(o => o.status === 'INTERESTED').length;
  const totalEarnings  = mockProcurementOrders
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <div className="space-y-3">
            {mockSoilTestBookings.slice(0, 3).map(booking => (
              <Link
                key={booking.id}
        ...
        <Card>
          ...
          <div className="space-y-3">
            {mockMitraBookings.slice(0, 2).map(session => (
              <div
                key={session.id}
        ...
        {mockProcurementOrders.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            {mockProcurementOrders.map(order => (
              <div key={order.id} className="flex items-center justify-between p-3 rounded-xl bg-amber-50 border border-amber-100">
                <div className="flex items-center gap-3">
